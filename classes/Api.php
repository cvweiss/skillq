...
class Api
{
     ...
     *
     * @static
     * @param int    $keyID The keyID to be checked.
     * @param string $vCode The vCode to be checked
    ...
    public static function addApi($keyID, $vCode)
    {
        ...
        if ($userID == null) {
            throw new Exception("Must be logged in to add an API");
        }

        $exists = Db::queryRow(
          "SELECT userID, keyID, vCode FROM skq_api WHERE keyID = :keyID AND vCode = :vCode",
        ...
        if ($exists == null) {
            // Insert the api key
            Db::execute(
              "INSERT INTO skq_api (userID, keyID, vCode) VALUES (:userID, :keyID, :vCode)",
              array(":userID" => $userID, ":keyID" => $keyID, ":vCode" => $vCode)
            );
        } else {
            if ($exists["userID"] == 0) {
    ...
    public static function deleteKey($keyID)
    {
        $userID = user::getUserID();
        Db::execute(
          "DELETE FROM skq_api WHERE userID = :userID AND keyID = :keyID",
          array(":userID" => $userID, ":keyID" => $keyID)
        );
    ...
    public static function getKeys()
    {
        $userID = user::getUserID();
        $result = Db::query(
          "SELECT keyID, vCode, label, lastValidation FROM skq_api WHERE userID = :userID order by keyID",
          array(":userID" => $userID),
          0
        );
    ...
    public static function getCharacterKeys()
    {
        $userID = user::getUserID();
        $result = Db::query(
          "select c.* from skq_api_characters c left join skq_api a on (c.keyID = a.keyID) where a.userID = :userID",
          array(":userID" => $userID),
          0
        );
    ...
    public static function getCharacters()
    {
        $userID  = user::getUserID();
        $db      = Db::query(
          "SELECT characterID FROM skq_api_characters c left join skq_api a on (c.keyID = a.keyID) where userID = :userID",
          array(":userID" => $userID),
          0
        );
    ...
    public static function processApi($keyRowID, $keyID = null, $vCode = null)
    {
        if ($keyID == null || $vCode == null) {
            $row   = Db::queryRow(
              "select keyID, vCode from skq_api where keyRowID = :keyRowID",
        ...
        if ($keyRowID == null) {
            $row      = Db::queryRow(
              "select keyRowID from skq_api where keyID = :keyID and vCode = :vCode and errorCode = 0",
              array(":keyID" => $keyID, ":vCode" => $vCode),
              0
            );
            $keyRowID = $row["keyRowID"];
        }
        if ($keyID == null || strlen($keyID) == "") {
            throw new Exception("Invalid keyID! :$keyRowID:$keyID:$vCode:");
        }
        if ($vCode == null || strlen($vCode) == "") {
            throw new Exception("Invalid vCode! :$keyRowID:$keyID:$vCode:");
        }
        $pheal = Util::getPheal($keyID, $vCode);
        try {
            Db::execute(
              "update skq_api set errorCode = 0, accessMask = 0, lastValidation = now() where keyRowID = :keyRowID",
              array(":keyRowID" => $keyRowID)
            );
            $result     = $pheal->accountScope->APIKeyInfo();
            ...
            Db::execute(
              "update skq_api set accessMask = :aM, expires = :expires, cachedUntil = :cachedUntil where keyRowID = :keyRowID",
              array(":keyRowID"    => $keyRowID,
                    ":aM"          => $accessMask,
                    ":expires"     => $expires,
            ...
            if ($accessMask & 33554432) {
                ...
                Db::execute(
                  "replace into skq_api_account (keyRowID, paidUntil, createDate, logonCount, logonMinutes)
            ...
            $code = $ex->getCode();
            Db::execute(
              "update skq_api set errorCode = :code where keyRowID = :keyRowID",
              array(":keyRowID" => $keyRowID, ":code" => $code)
            );
        ...
        foreach ($chars as $char) {
            ...
            $allToons[] = $charID;
            Db::execute(
              "insert ignore into skq_character_info (keyRowID, characterID, characterName) values (:keyRowID, :charID, :name)",
              array(":keyRowID" => $keyRowID, ":charID" => $charID, ":name" => $name)
            );
        }
        if (sizeof($allToons) == 0) {
            Db::execute(
            ...
            Db::execute(
              "delete from skq_character_info where keyRowID = :keyRowID and characterID not in (" . implode(
                ",",
                $allToons
              ) . ")",
              array(":keyRowID" => $keyRowID)
