...
class Info
{
	...
	public static function getRefTypeName($id) {
		return Db::queryField("select refTypeName from api_refTypes where refTypeID = :id", "refTypeName", array(":id" => $id));
	}
	...
	public static function addInfo(&$element)
	{
		...
		foreach ($element as $key => $value) {
			if (is_array($value)) $element[$key] = Info::addInfo($value);
				...
				break;
				case "subFlag":
					if ($value == 0) $element["subStatus"] = "";
					if ($value == 1) $element["subStatus"] = "Subscription Expiring...";
					if ($value == 2) $element["subStatus"] = "Subscription Expired!";
				break;
