...
if (isset($bypassLogin)) {
	$charInfo = Db::queryRow("select * from skq_character_info i where characterName = :name", array(":name" => $name), 1);
	$isShare = true;
} else {
	$bypassLogin = false;
	if (!User::isLoggedIn()) $app->redirect("/login/");
	$userID = User::getUserID();

	$charInfo = Db::queryRow("select i.* from skq_api a left join skq_character_info i on (a.keyRowID = i.keyRowID) where a.userID = :userID and characterName = :name", array(":userID" => $userID, ":name" => $name), 1);
	$isShare = false;
}
if (!$charInfo) $app->redirect("/");

if (!isset($pageType)) $pageType = "overview";
else $pageType = strtolower($pageType);


$charID = $charInfo["characterID"];
$training = Db::queryRow("select * from skq_character_training where characterID = :charID and trainingEndTime > now()", array(":charID" => $charID), 1);
if ($training == null or sizeof($training) == 0) Db::queryRow("select * from skq_character_queue where characterID = :charID and endTime > now() order by queuePosition limit 1", array(":charID" => $charID), 1);
$skills = Db::query("select s.typeID, i.typeName, s.level, s.training, s.queue, g.groupID, g.groupName, s.skillPoints from skq_character_skills s left join ccp_invTypes i on (s.typeID = i.typeID) left join ccp_invGroups g on (i.groupID = g.groupID) where characterID = :charID order by g.groupName, i.typeName", array(":charID" => $charID), 1);
$queue = Db::query("select typeID, level, startTime, endTime from skq_character_queue where characterID = :charID and endTime >= now()", array(":charID" => $charID), 1);
...
$skillTrain = array();
if ($pageType == "train") {
	$implants = Db::query("select * from skq_character_implants where characterID = :charID order by (baseValue + bonus) desc, baseValue desc, bonus desc, attributeID", array(":charID" => $charID), 1);
	$attributes = array();
	...
	do {
		...
		foreach($skillTrain as $skill) {
			$typeID = $skill["typeID"];
			if (isset($preReqs[$typeID])) continue;
			$processed++;
			...
			if (!isset($timedSkills[$time])) $timedSkills[$time] = array();
...
Info::addInfo($wallet);
$maxSeconds = 0;
foreach($queue as $skill) $maxSeconds = max($maxSeconds, $skill["endTimeSeconds"]);
$pageRefresh = 3600;
@$seconds = $charInfo["cachedUntilSeconds"];
if ($seconds > 0) $pageRefresh = $seconds;
...
$app->render("char.html", array("char" => $charInfo, "training" => $training, "skills" => $skills, 
			"queue" => $queue, "pageRefresh" => $pageRefresh, "queueFinishes" => $maxSeconds,
			"groupSP" => $groupSP, "totalSP" => $totalSP, "wallet" => $wallet, "pageType" => $pageType,
			"isShare" => $isShare, "skillTrain" => $skillTrain));
